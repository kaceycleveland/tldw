...
function showLoadingIndicator() {
  ...
  loader.className = 'fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-[10002] font-sans'
  
  loader.innerHTML = `
    <div class="bg-white p-8 px-10 rounded-xl text-center shadow-2xl">
      <div class="w-10 h-10 border-4 border-gray-200 border-t-blue-500 rounded-full animate-spin mx-auto mb-5"></div>
      <h3 class="m-0 mb-2 text-gray-800 text-lg font-semibold">Analyzing Video</h3>
      <p class="m-0 text-gray-500 text-sm">Extracting exercises using AI...</p>
    </div>
  `
...
function displayExerciseResults(exercises: any[]) {
  ...
  resultsContainer.className = 'fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white border-2 border-blue-500 rounded-xl p-5 max-w-3xl max-h-[80vh] overflow-y-auto z-[10001] font-sans shadow-2xl'
  
  // Create header
  const headerHTML = `
    <div class="flex justify-between items-center mb-4 border-b border-gray-200 pb-3">
      <h3 class="m-0 text-gray-800 text-lg font-bold">Exercise Analysis Results (${exercises.length} exercises found)</h3>
  ...
  if (exercises.length === 0) {
    exercisesHTML = '<p class="text-center text-gray-500 italic">No exercises found in this video.</p>'
  } else {
